...
const main = require("./main-D4le4Kvs.js");
...
var bind = main.functionBind;
var hasOwn$2 = main.hasown;
...
const FormData$2 = /* @__PURE__ */ main.getDefaultExportFromCjs(form_data);
...
var debug_1 = function() {
  if (!debug$1) {
    try {
      debug$1 = main.srcExports("follow-redirects");
    } catch (error) {
    }
...
const followRedirects = /* @__PURE__ */ main.getDefaultExportFromCjs(followRedirectsExports);
...
const textEncoder = typeof TextEncoder === "function" ? new TextEncoder() : new require$$0$1.TextEncoder();
const CRLF = "\r\n";
  ...
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    if (utils$1.isSpecCompliantForm(data)) {
      ...
      if (!headers.hasContentLength()) {
        try {
          const knownLength = await require$$0$1.promisify(data.getLength).call(data);
...
class LicenseService {
  constructor() {
    ...
    __publicField(this, "licenseServerUrl");
    __publicField(this, "machineId");
    this.store = new main.Store({
      name: "license-store",
    ...
    this.machineId = nodeMachineId.machineIdSync();
...
-----END PUBLIC KEY-----`;
  ...
  async deactivateLicense() {
    try {
      ...
      main.log.info(`Deactivating license on this machine: ${this.machineId}`);
      ...
      main.log.error("License deactivation error:", error);
      return {
        success: false,
  ...
  async validateLicense(licenseKey) {
    var _a, _b;
    try {
      main.log.info(`Validating license with server: ${this.licenseServerUrl}`);
      ...
      const licenseData = {
        ...
        signature: "stripe-validated",
        // Using Stripe validation instead of RSA
      ...
      const daysRemaining = Math.floor((expirationDate.getTime() - now.getTime()) / (1e3 * 60 * 60 * 24));
      return {
        isValid: true,
        isExpired,
      ...
      main.log.error("License validation error:", error);
      ...
      return {
        isValid: false,
        isExpired: false,
        error: ((_b = (_a = error.response) == null ? void 0 : _a.data) == null ? void 0 : _b.error) || "License validation failed"
      };
    }
  }
  async validateOffline() {
    ...
    if (lastValidationDate) {
      const daysSinceValidation = Math.floor((now.getTime() - lastValidationDate.getTime()) / (1e3 * 60 * 60 * 24));
  ...
  async storeLicense(licenseData) {
    ...
    this.store.set("lastValidation", (/* @__PURE__ */ new Date()).toISOString());
  ...
  async getCurrentLicense() {
    ...
    if (!license) return null;
    const storedFingerprint = this.store.get("fingerprint");
    const currentFingerprint = await this.generateHardwareFingerprint();
    if (storedFingerprint !== currentFingerprint) {
      main.log.warn("Hardware fingerprint mismatch - license may have been moved");
    }
    return license;
  }
  ...
  async generateHardwareFingerprint() {
    ...
    try {
      ...
      const data = {
        ...
        arch: process.arch,
        cpuModel: ((_a = cpus[0]) == null ? void 0 : _a.model) || "unknown",
        totalMemory: os.totalmem(),
        // MAC addresses (excluding virtual interfaces)
        macs: Object.values(networkInterfaces).flat().filter((iface) => iface && !iface.internal && iface.mac !== "00:00:00:00:00:00").map((iface) => iface.mac).sort()
      };
      const hash = crypto$1.createHash("sha256");
      hash.update(JSON.stringify(data));
      return hash.digest("hex");
    } catch (error) {
      main.log.error("Failed to generate hardware fingerprint:", error);
    ...
    try {
      ...
      if (license.stripeSubscriptionId) {
        try {
          ...
          main.log.error("Failed to fetch subscription status:", error);
        }
      }
      return {
      ...
      main.log.error("Failed to get license info:", error);
      return {
        isLicensed: false
      };
    }
  }
  async getLicenseInfoWithoutAuth() {
    try {
      const license = this.store.get("license");
      if (!license) {
        return {
      ...
      main.log.error("Failed to get license info without auth:", error);
      return {
        isLicensed: false
      };
    }
  }
  async createCheckoutSession(email, planType) {
    var _a, _b;
    try {
      ...
      main.log.error("Failed to create checkout session:", error);
      return {
        error: ((_b = (_a = error.response) == null ? void 0 : _a.data) == null ? void 0 : _b.error) || "Failed to create checkout session"
      };
    }
  }
  async getSubscriptionStatus(licenseKey) {
    try {
      ...
      main.log.error("Failed to get subscription status:", error);
      return null;
    }
  }
  async createPortalSession(licenseKey) {
    var _a, _b;
    try {
      ...
      main.log.error("Failed to create portal session:", error);
      return {
        error: ((_b = (_a = error.response) == null ? void 0 : _a.data) == null ? void 0 : _b.error) || "Failed to create portal session"
      };
    }
  }
  // Check if license is expiring soon (within 30 days)
  async shouldShowExpirationWarning() {
    const status = await this.checkLicenseStatus();
    if (!status.isValid) {
      return { show: false };
    }
